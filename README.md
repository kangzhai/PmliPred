...
Supplementary note：In the original datasets, miRNA and lncRNA sequences are spliced. Given that many researchers have asked me for datasets with the separation of miRNA and lncRNA sequences, I updated "Datasets.zip" on March 24, 2021. The "Sequence(separated with commas).fasta" files are added, which separate miRNA and lncRNA sequences with commas. When executing PmliPred, the original datasets are still used. The "Sequence(separated with commas).fasta" files provide another option for the related research.
